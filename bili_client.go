...
type GuardTabTopListResp struct {
	Info struct {
		Num                     int `json:"num"`
		Page                    int `json:"page"`
		Now                     int `json:"now"`
	...
	RemindMsg string `json:"remind_msg"`
	Typ       int    `json:"typ"`
}

func (b *BiliClient) GuardTabTopList(roomID, rUID, page, pageSize int64) (*GuardTabTopListResp, error) {
	resp, err := b.RawParse(
		BiliLiveURL,
		...
		map[string]string{
			"roomid":    strconv.FormatInt(roomID, 10),
			"ruid":      strconv.FormatInt(rUID, 10),
